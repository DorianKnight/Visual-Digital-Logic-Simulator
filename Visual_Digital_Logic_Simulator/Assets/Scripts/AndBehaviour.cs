...
public class AndBehaviour : MonoBehaviour
{
    private GameObject AndSpawner;
    ...
    void Start()
    {
        AndSpawner = GameObject.Find("AndSpawner");
    }
    ...
    {
        if (Input.GetMouseButtonDown(0) && AndSpawner.GetComponent<SpawnAnd>().pickedUp == false && AndSpawner.GetComponent<SpawnAnd>().getChipConnectedToMouse() == null)
        {
            // If there isn't already a chip connected to the mouse
            AndSpawner.GetComponent<SpawnAnd>().setChipConnectedToMouse(GetComponent<Transform>());
